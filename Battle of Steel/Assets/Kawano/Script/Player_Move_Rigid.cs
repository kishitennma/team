...
public class Player_Move_Rigid : MonoBehaviour
{
    ...
    //UnityのRigidBodyを使った場合のプレイヤーの移動方法
    ...
    void Update()
    {
        ...
        if(!Physics.Raycast(transform.position,move_dir,move_distance + 0.1f))
        {
            //移動速度を設定
            ...
            else
            {
                ...
                input_direction = transform.right * h + transform.forward * dash_speed;//移動ベクトルを設定
            }
